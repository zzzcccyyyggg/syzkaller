#!/usr/bin/env python3
"""Prepare or launch the local PTMX four-arm, 24-hour formal experiment."""
...
ARMS = tuple(
    Arm(label, variant, fixed_us, index)
    for index, (label, variant, fixed_us) in enumerate((
        ("dynamic", "dynamic", 0),
        ("random", "random", 0),
        ("fixed50", "fixed", 50),
...
def main() -> int:
    ...
    for arm in ARMS:
        ...
        run_id = f"{args.run_prefix}-{arm.label}{suffix}"
        ...
        session = f"local24-ptmx-{arm.label}"
        if subprocess.run(
