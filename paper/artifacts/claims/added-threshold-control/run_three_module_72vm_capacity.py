#!/usr/bin/env python3
"""Prepare or launch the 12-arm, 72-VM remote capacity smoke."""
...
ARMS = tuple(
    Arm(module, label, variant, fixed_us, module_index * 4 + arm_index)
    for module_index, module in enumerate(("floppy", "bt-stack", "f2fs"))
    for arm_index, (label, variant, fixed_us) in enumerate((
        ("dynamic", "dynamic", 0),
        ("random", "random", 0),
        ("fixed50", "fixed", 50),
...
def main() -> int:
    ...
    parser.add_argument("--run-prefix", default="20260826-remote-cap72-v1")
    parser.add_argument(
        "--manager-bin",
